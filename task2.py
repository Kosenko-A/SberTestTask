...
for num, iss_d, stat, code, cli, bill, date, curr1, amount1 in zip(no, iss_date, status, IE_code, client, bill_ref_no,
    ...
    date_for_url = new_date
    url = "https://www.cbr.ru/currency_base/daily/?UniDbQuery.Posted=True&UniDbQuery.To=" + date_for_url
    page = requests.get(url)
    soup = BeautifulSoup(page.text, "html.parser")
    allString = soup.findAll('tr')
    usd_course = 0
    for i in range(1, len(allString)):
        td_list = allString[i].find_all('td')
        if td_list[1].text == 'USD':
            usd_course = float(td_list[4].text.replace(',', '.'))
    usd_cal = str(round((float(amount1) / usd_course), 2))

    usd = ET.SubElement(ecert, 'SVALUEUSD')
    usd.text = str(usd_cal)

indent(root)
ET.ElementTree(root).write('my_output2.xml', encoding='utf-8', method='xml', xml_declaration=True)
